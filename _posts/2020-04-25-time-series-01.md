---
title: "[시계열데이터분석] 비트코인 시세 예측_1편"
date: 2020-04-25
categories: time-serise bitcoin ARIMA Additive model machine-learning data-science update
---
...
즉, 관측치가 시간적 순서를 가진 데이터로 변수간의 상관성이 존재하는 데이터를 다루며, 심장 박동 데이터처럼 1) 규칙적 시계열 데이터를 분석하는 것과 비트코인 시세 예측과 같이 2) 불규칙적인 데이터를 분석하는 것으로 구분할 수 있습니다.
...
![비트코인 데이터 다운로드 화면](_posts/images/2020-04-25-time-series-01/1_1.png)

*간혹 확장자가 없는 파일로 다운로드되는데, 확장자 .csv를 붙여주기만 하면 사용할 수 있습니다.*

본 포스팅에서 사용하는 학습 데이터는 3년간 시세 데이터([train.csv](assets/train.csv)) , 테스트 데이터는 7일간 시세 데이터([test.csv](assets/test.csv))를 사용하였습니다. 데이터셋 환경 구축이 완료되었다면 데이터를 로드해보겠습니다.
...
![1_2](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_2.png)
...
| ----------------------- | :----------------------------------------------------------: | :----------------------------------------------------------: |
| **Correlation** **> 0** | X가 양의 값이고 증가하는 추세일 때, <\br> Y는 양의 값이고 증가하는 추세 | X가 양의 값이고 증가하는 추세일 때, <\br> Y는 양의 값이고 감소하는 추세 |
| **Correlation** **< 0** | X가 양의 값이고 증가하는 추세일 때, <\br> Y는 음의 값이고 증가하는 추세 | X가 양의 값이고 증가하는 추세일 때, <\br> Y는 음의 값이고 감소하는 추세 |
...
![1_3](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_3.png)

![1_4](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_4.png)
...
![1_5](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_5.png)

![1_6](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_6.png)

![1_7](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_7.png)
...
![1_8](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_8.png)
...
![1_9](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_9.png)
...
![1_11](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_11.png)
...
![](assets/[시계열데이터분석] 비트코인 시세 예측_1편/1_10.png)
